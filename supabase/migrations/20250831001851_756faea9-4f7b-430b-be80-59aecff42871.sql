-- Security Fix 1: Secure public data access in tenants table
-- Remove overly permissive public access to tenants table
DROP POLICY IF EXISTS "Public access to domains" ON public.domains;

-- Create restricted public access for domains (only active domains)
...
-- Security Fix 2: Secure service health data
-- Remove public access to service health data
DROP POLICY IF EXISTS "Public can view service health" ON public.service_health_status;
DROP POLICY IF EXISTS "Public can view services" ON public.services;

-- Create admin-only access for service health data
...
CREATE POLICY "Admins can manage service health" ON public.service_health_status
FOR ALL 
USING (has_employee_role('ADMIN'::employee_role))
WITH CHECK (has_employee_role('ADMIN'::employee_role));

-- Security Fix 3: Secure background jobs access
-- Ensure only admins can view background jobs
DROP POLICY IF EXISTS "Public can view jobs" ON public.background_jobs;

-- Security Fix 4: Create secure public view for tenant information
...
-- Security Fix 5: Harden database functions with proper search_path
...
-- Security Fix 6: Secure API keys table
-- Ensure proper isolation of API keys
...
-- Security Fix 7: Create function to validate sensitive operations
...
-- Security Fix 8: Create security monitoring trigger
...
DROP TRIGGER IF EXISTS monitor_tenants_access ON public.tenants;
CREATE TRIGGER monitor_tenants_access
  BEFORE INSERT OR UPDATE OR DELETE ON public.tenants
  FOR EACH ROW EXECUTE FUNCTION monitor_tenants_access();
...
-- Security Fix 9: Update admin access policy to be more restrictive
...
-- Security Fix 10: Add rate limiting for sensitive operations
