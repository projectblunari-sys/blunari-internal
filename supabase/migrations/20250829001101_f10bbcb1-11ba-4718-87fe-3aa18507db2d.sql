...
WITH CHECK (
  auth.uid() = id AND
  -- Only allow updates to safe fields, never role
...
-- 3. Secure business_metrics table - Remove overly permissive policy
...
-- 5. Secure API key policies - Prevent hash exposure
...
-- 7. Add role assignment validation trigger
...
BEGIN
  ...
  IF OLD.role IS DISTINCT FROM NEW.role THEN
    IF NOT validate_role_assignment(assigner_role, NEW.role) THEN
...
DROP TRIGGER IF EXISTS employee_role_validation ON public.employees;
CREATE TRIGGER employee_role_validation
  BEFORE UPDATE ON public.employees
  FOR EACH ROW
  EXECUTE FUNCTION public.validate_employee_role_change();

-- 8. Secure domain tables with proper tenant isolation
UPDATE public.domains SET status = 'pending' WHERE verification_status = 'pending';

-- 9. Add rate limiting table for security events
...
-- 10. Add function to check and enforce rate limits
...
BEGIN
  ...
  IF current_count IS NULL THEN
    ...
    INSERT INTO public.security_rate_limits (identifier, event_type, window_start)
    VALUES (p_identifier, p_event_type, window_start);
    RETURN true;
