  ...
  try {
    ...
    switch (action) {
      case 'restaurants': {
        ...
        // Enhance data with payment info
        ...
        logStep("Fetched restaurants", { count: enhancedTenants?.length });
      ...
      case 'payment-history': {
        ...
        logStep("Fetched payment history", { count: payments?.length, tenantId });
      ...
      case 'analytics': {
        ...
        const analytics = {
          ...
          revenueByPlan: revenue?.reduce((acc, r) => {
            // You'd need to join with subscription data to get the actual plan
